...
models:
  ...
  - id: "STATE-003"
    entity: "INFO-009"
    name: "事務局招待ステータス"
    description: "事務局招待のライフサイクル。承諾待ちから、承諾・辞退・取り消しのいずれかで終わる。期限切れは状態として持たず、expires_at で判定する（団体の招待と同じ）。"
    states:
      - name: "承諾待ち"
        status_value: "pending"
        description: "事務局が招待を送った直後の状態。有効期限内であれば、宛先本人が承諾・辞退できる（COND-015）。"
      - name: "承諾済み"
        status_value: "accepted"
        description: "宛先本人が承諾した状態。INFO-006.is_staff が true になっている。"
      - name: "辞退"
        status_value: "rejected"
        description: "宛先本人が辞退した状態。"
      - name: "取り消し"
        status_value: "canceled"
        description: "承諾前に事務局が取り消した状態。"
    transitions:
      - from: "[*]"
        to: "承諾待ち"
        trigger: "UC-026（事務局が招待）"
        condition: "COND-015: 宛先がすでに事務局でなく、宛先に承諾待ちかつ期限内の事務局招待が無いこと"
      - from: "承諾待ち"
        to: "承諾済み"
        trigger: "UC-027（宛先本人が承諾）"
        condition: "COND-015: 期限内で、ログイン中のメールアドレスが宛先と一致すること"
      - from: "承諾待ち"
        to: "辞退"
        trigger: "UC-027（宛先本人が辞退）"
        condition: "COND-015: 期限内で、ログイン中のメールアドレスが宛先と一致すること"
      - from: "承諾待ち"
        to: "取り消し"
        trigger: "UC-026（事務局が取り消し）"
        condition: "事務局のみ実行できる。取り消せるのは送った本人に限らない"
...
承諾と辞退ができるのは、期限内（送信から48時間）で、ログイン中のメールアドレスが宛先と一致する場合に限る（COND-015）。期限切れは状態として持たず、承諾待ちのまま expires_at で判定する。承諾すると INFO-006.is_staff が true になる。事務局権限の剥奪（UC-028）は INFO-006.is_staff を false に戻す操作であり、事務局招待の状態は変えない。招待を送った事務局が剥奪されても、その人が送った承諾待ちの招待は有効なまま残る（COND-015）。
