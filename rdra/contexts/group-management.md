...
system:
  ...
  conditions:
    ...
    - id: "COND-011"
      name: "団体情報の存在秘匿"
      description: |
        所属していない団体について、その団体が存在するかどうかを判別できる情報を返さない。
        団体を指定する操作は、指定された団体が存在しない場合と、存在するが操作者が所属していない場合とで、
        同じ応答を返す。団体の識別子や表示名を総当たりすることで、どの団体が登録されているかを
        推測できてはならない。事務局（COND-009）はこの制限を受けない。
      traces_to: ["UC-011", "UC-012", "UC-013", "UC-014", "UC-022"]
  variations:
    - id: "VAR-001"
      name: "メンバーロール"
...
団体の一覧・詳細は所属しているメンバーと事務局にしか見せない。加えて、所属していない団体については存在の有無自体を伏せる（COND-011）。学生団体の登録の有無は、その団体が活動しているかどうかを外部に示す情報になりうるためである。
