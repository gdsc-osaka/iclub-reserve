...
boundary:
  usecases:
    ...
    - id: "UC-032"
      ...
      events: []
      traces_to: ["BUC-001"]
      description: "自分が所属する団体の予約を一覧で確認する。所属する団体が複数あるときは1団体ずつ切り替えて表示する。所属していない団体を指定されても、その団体の予約は表示せず、所属する団体のうち先頭の団体を表示する（COND-008）。状態（すべて・仮予約・承認済み・終了）、期間（これから・過去。終了時刻を過ぎたものを過去とする）、施設/設備で絞り込める。既定は「すべて」の「これから」で、開始日時の近い順に並べる。仮予約に同じ施設・時間帯の承認済み予約や他の仮予約が重なっていれば、承認されない可能性がある旨を表示する（COND-001）。各予約から取り消し（UC-003）・キャンセル（UC-004）を行い、予約詳細（UC-034）を開ける。団体の状態を問わず閲覧できる（COND-006）。"
    - id: "UC-034"
      name: "予約の詳細を確認する"
