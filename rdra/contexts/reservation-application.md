...
boundary:
  ...
  screens:
    ...
    - id: "SCR-005"
      name: "予約詳細・メッセージ画面"
      description: "予約詳細の確認・変更、および事務局と団体間のメッセージをやり取りする画面。全項目とメッセージを表示するのは自団体のメンバーと事務局のみ。他団体のユーザーが開いた場合は公開範囲（団体名・施設・日時・ステータス）だけを表示し、メッセージ欄は表示しない（COND-008）。その予約の操作履歴（UC-024・UC-025）も表示する。履歴欄を表示するのは自団体のメンバーと事務局のみで、団体側には事務局権限による操作の操作者を「事務局」と表示する（COND-012）。"
      information: ["INFO-001", "INFO-004", "INFO-008"]
