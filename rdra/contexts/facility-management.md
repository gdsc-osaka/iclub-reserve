...
value:
  ...
  requirements:
    ...
    - id: "REQ-023"
      ...
      traces_to: ["GOAL-001"]
    - id: "REQ-024"
      description: "施設・設備を無効化できる。無効化前に将来の予約（仮予約・承認済み）がすべて取り消し・キャンセル済みであることが必要。現在進行中の予約は除く。"
      traces_to: ["GOAL-002"]
    - id: "REQ-025"
...
system:
  ...
  conditions:
    - id: "COND-003"
      name: "施設無効化の前提条件"
      description: "無効化対象施設の将来の予約（仮予約・承認済み）がすべて取り消し・キャンセル済みであること。ただし start_at が現在時刻以前かつ end_at が現在時刻以降の承認済み予約（現在使用中）は対象外とする。"
      traces_to: ["UC-016"]
  variations: []
---
