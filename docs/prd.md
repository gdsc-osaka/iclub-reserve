...
> **前回からの変更**: パスキーの一覧に提供元のアイコンを出し、提供元の対応表をコミュニティの一覧の写しに改めた（REQ-043・COND-020）。メールアドレス変更のダイアログ（SCR-017）は外側を押しても閉じないようにした。  
...
- メールアドレス変更のダイアログ（SCR-017）は、外側を押しても閉じず、閉じるボタンも置かない。途中でやめるときは「キャンセル」で閉じる。誤って閉じると、届いた認証コードを使えずに最初からやり直すことになるためである。
