...
```
メールアドレス・氏名・パスワード入力 + 利用規約同意
  → ドメイン検証（osaka-u.ac.jp）
  → 認証コードをメール送信
  → コード入力で本登録完了
```
...
```
[新規] ──────────────────────────────────────────────────────────────
  │ 仮予約申請（UC-002）       事務局直接作成（UC-008）
  ▼                              ▼
仮予約（provisional）         承認済み（approved）
  │                              │
  ├──[事務局承認 UC-006]──────►承認済み（approved）
  │                              │
  ├──[団体取り消し UC-003]────►取り消し済み（withdrawn）─► [終了]
  │                              │
  ├──[事務局却下 UC-006]──────►却下済み（rejected）──────► [終了]
  │                              │
  │                 ┌────────────┤
  │                 │            │
  │           [施設・日時変更    ├──[団体キャンセル UC-004]──►キャンセル済み（cancelled）─► [終了]
  │            UC-005]           │
  │                 │            └──[事務局キャンセル UC-007]──►事務局キャンセル済み（cancelled_by_staff）─► [終了]
  │                 ▼
  └────────────仮予約（provisional）
```
...
```
ユーザー（INFO-006）
  └─ 所属する ─► メンバーシップ（INFO-005）◄─ 構成される ─ 団体（INFO-003）
                                                                  │
                                                              申請する
                                                                  │
施設/設備（INFO-002）──予約される──►予約（INFO-001）◄──────────────┘
                                         │
                                     持つ │
                                         ▼
                                   メッセージ（INFO-004）
