...
> **ステータス**: Draft  
> **最終更新**: 2026-09-23  
> **準拠する RDRA 成果物**: [`change-log.md`](../change-log.md) の 2026-09-23 エントリ時点  
> **前回からの変更**: 操作履歴（監査ログ, BIZ-007）と事務局権限の付与・剥奪（REQ-041）を追加。事務局による予約の削除を廃止（REQ-014）。  
...
**対象画面**: ログイン・新規登録画面（SCR-012）、認証コード入力画面（SCR-013）、初回セットアップ画面（SCR-014）、パスキー登録の案内画面（SCR-015）、メールアドレス変更画面（SCR-017）、事務局管理画面（SCR-019）、事務局招待の承諾画面（SCR-020）
...
- 認証・アカウントまわりの操作履歴（ログインの成否・メールアドレスの変更・パスキーの登録・氏名の変更）。不正利用・乗っ取りの調査は操作履歴の目的に含めない
- 閲覧の記録（操作履歴に残すのは変更のみ）
...
- 全要求: 41件（REQ-001〜041）
- ビジネスユースケース: 27件（BUC-001〜027）
- システムユースケース: 28件（UC-001〜028）
