...
> **前回からの変更**: アカウント設定画面（SCR-021）を追加し、氏名の変更（REQ-042）・パスキーの管理（REQ-043）・ログイン中の端末の管理（REQ-044）を定義。メールアドレスの変更（REQ-035）に変更後の処理（COND-018・EVT-016）を追加。  
...
| REQ-043 | 登録済みのパスキーを一覧で確かめ、追加・名前の変更・削除ができる。一覧には名前・登録日時・最後に使った日時・同期されるかどうかを表示する。追加には直近のログインを要する（COND-019）。最後の1つも削除でき、その後は認証コードでログインする                                                                                |
...
| パスキー         | 追加（COND-019）・名前の変更・削除（確認あり。最後の1つも可）               | 名前（COND-020）・登録日時・最後に使った日時・同期されるかどうか | UC-021・UC-030 |
...
事務局が仮予約の承認・却下、直接予約の作成・変更、メッセージ送受信を行う。
...
| REQ-024 | 施設・設備を無効化できる。無効化前に将来の予約（仮予約・承認済み）がすべて終了状態（取り消し済み・却下済み・キャンセル済み）であることが必要。現在使用中の承認済み予約は除外されるが、終了前の仮予約が残っている場合は無効化できない |
...
| COND-020 | **パスキーと端末の表示名**: 端末名は User-Agent から「Windows の Chrome」のように組み立てる。パスキーの名前は登録時にサーバーが付け、aaguid から提供元が分かればその名前、分からなければ登録した端末の端末名とする。名前は変更でき、COND-017 と同じ長さの規則に従う。ブラウザから名前を渡すと OS の保存ダイアログのアカウント名まで変わるため、サーバー側で付け、ブラウザから送られた名前は使わない                  |
...
- 全要求: 44件（REQ-001〜044）
- ビジネスユースケース: 30件（BUC-001〜030）
- システムユースケース: 31件（UC-001〜031）
