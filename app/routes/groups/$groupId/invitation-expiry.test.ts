...
describe("formatRemainingTime", () => {
  ...
  it("1 時間以上の場合は「あと N 時間で期限切れ」を返す", () => {
    // ちょうど 1 時間
  ...
  it("24 時間を超えても日に切り替えず、時間のまま表す", () => {
    // ちょうど 24 時間
    const exactly24h = new Date("2026-09-22T12:00:00.000Z");
    expect(formatRemainingTime(exactly24h, baseNow)).toBe("あと 24 時間で期限切れ");

    // 招待の有効期限いっぱい (48 時間)
    const exactly48h = new Date("2026-09-23T12:00:00.000Z");
    expect(formatRemainingTime(exactly48h, baseNow)).toBe("あと 48 時間で期限切れ");
  });
